...
def create_app(settings=None, factory=None, redis_client=None):
    ...
    app = FastAPI(title='MRMBURU TRADER', version='0.4.0', lifespan=lifespan)

    @app.exception_handler(CTraderAuthRequired)
    async def ctrader_auth_error(_, exc):
    ...
    @app.get('/health')
    def health():
        return {'status':'ok','mode':'paper','execution_enabled':False,'version':'0.4.0'}
    ...
    def paper_run(account_id:str, body:PaperRunRequest, s=Depends(db)):
        if account_id == settings.paper_account_id:
            raise HTTPException(409, 'This account uses the v0.4 ledger; legacy pipeline is disabled for it')
        feed = _feed()
        ai_opinion = None
        if body.include_ai:
            try:
    ...
    @app.get('/paper/v04/status', dependencies=[Depends(research)])
    def simulator_status(s=Depends(db)):
        from core.models import PaperLedger, PaperEvent
        row = s.get(PaperLedger, settings.paper_account_id) if settings.paper_account_id else None
        events = s.scalars(select(PaperEvent).where(PaperEvent.account_id == settings.paper_account_id)
            .order_by(PaperEvent.created_at.desc(), PaperEvent.id.desc()).limit(100))
        def cached(key):
    ...
    def enable_simulator_account(body:Control, s=Depends(db)):
        account = s.scalar(select(Account).where(Account.id == settings.paper_account_id).with_for_update())
        if account is None or account.platform != 'local-paper' or account.currency != 'USD':
            raise HTTPException(409, 'Configure a dedicated local-paper USD account first')
        account.enabled = True
        s.add(AuditEvent(actor='admin', action='paper_v04.enable',
            payload={'account_id': account.id, 'reason': body.reason}))
        return {'enabled':True, 'execution_enabled':False, 'global_stop_unchanged':True}

    @app.post('/paper/v04/acknowledge-gap', dependencies=[Depends(admin)])
    def acknowledge_gap(body:Control, s=Depends(db)):
        from core.models import PaperLedger, PaperEvent
        account = s.scalar(select(Account).where(Account.id == settings.paper_account_id).with_for_update())
        row = s.get(PaperLedger, account.id) if account else None
        if row is None or row.state.get('position'):
            raise HTTPException(409, 'A flat paper ledger is required before acknowledging a gap')
        row.state = {**row.state, 'paused':None}
        s.add(PaperEvent(account_id=account.id, payload={'kind':'gap_acknowledged', 'reason':body.reason}))
        return {'paused':None, 'execution_enabled':False}
